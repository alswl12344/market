...
<body>
	...
	<section class="content">
		...
		<Form method="post">
			...
			생년월일 : <input type="text" name="birth" id="birth" /><br> 연락처 :
			<input type="text" name="phone" id="phone" /><br> 
			<input type="button" value="아이디찾기" id="btnSearchId" />
		</Form>
				...
				$.ajax({
					...
					success : function(result) { // 사용자가 정하는 임의의 변수
						...
						if (result == null) { // result값의 크기를 조건으로 댓글이 있는지 확인  / 리턴된 값이 있다 => 길이가 1보다 크다
							htmls = htmls + ("<h3>성공 등록된 아이디가 없습니다.</h3>");
						} else {
					...
					error : function(msg) {
						console.log(msg);
						alert("아이디 찾기 실패");
						htmls = htmls + ("<h3>실패 등록된 아이디가 없습니다.</h3>");
						$("#answer").html(htmls)
					}
				});
